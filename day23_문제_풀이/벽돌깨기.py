...
def shoot(level, remains, current_arr):
    global min_blocks
    ...
    for col in range(W):
        ...
        # 2. 원본 리스트에 col 위치에 구슬을 쏜다
        # 3. level + 1번 상태로 이동 (다음 재귀 호출)
        # 4. col 위지에 쏘기 전 상태로 복구

        # 방법 2. (복구하는 시간이 너무 오래 걸림 -> 개선)
        # 1. col 위치에 쏘기 전 상태를 복사
        # 2. 복사한 리스트의 col 위치에 구슬을 쏜다.
        # 3. level + 1번 상태로 이동 (다음 재귀 호출) + col 위치에 쏘고 난 상태를 함께 전달
        copy_arr = [row[:] for row in current_arr]

        # col 위치에 구슬을 쏘자!
        # 1. 첫번째 만나는 벽돌 위치 (구슬을 쏘는 열에서 가장 위에 있는 벽돌 찾기)
        row = -1  # 없다고 가정
...
for t in range(1, T + 1):
    ...
    # 1. 최소 벽돌
    # - 몇 개 남았을까? 계산해야 한다. -> 2차원 리스트를 순회하면서 매번 계산하면 너무 느림
    # - 현재 남은 벽돌 수를 통해 해결됨!
    # 2. 현재 벽돌이 다 깨지면(남은 벽돌이 없음) 더 이상 진행할 필요가 없음
    # - 현재 벽돌 수도 같이 저장하면 좋을 것 같다.
    # 3. N번 구슬 쏘기
    # - 시작점 : 0번
